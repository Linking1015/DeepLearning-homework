...
class MyNet_d(nn.Module):
    def __init__(self, input_shape, num_big_classes):
        ...
        self.flatten = nn.Flatten()
        self.dropout = nn.Dropout(0.5)
        self.fc1 = nn.Linear(32 * (input_shape[1] // 8) * (input_shape[2] // 8), 128)
...
class MyNet_w(nn.Module):
    def __init__(self, input_shape, num_big_classes):
        super(MyNet_w, self).__init__()
        self.conv1 = nn.Conv2d(in_channels=input_shape[0], out_channels=16, kernel_size=3, stride=1, padding=1)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.conv2 = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=3, stride=1, padding=1)
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)
        ...
        self.flatten = nn.Flatten()
        self.dropout = nn.Dropout(0.5)
        self.fc = nn.Linear(64 * (input_shape[1] // 4) * (input_shape[2] // 4), num_big_classes)
...
# --- 2. 训练函数  ---
def train_single_model(model_name, train_loader, test_loader, device, num_epochs, fixed_seed, my_model):
    """
    ...
    model = my_model(input_shape=(3, 32, 32), num_big_classes=10).to(device)
...
epochs = 50
Model_setting = [MyNet, MyNet_d, MyNet_w]
results = []

# === 循环不同的 Epoch 设置 ===
for m in Model_setting:
    model_name = f"{m.__name__}"
