...
def train_single_model(model_name, train_loader, test_loader, device, num_epochs, fixed_seed):
    """
    ...
    # 1. 关键：每次训练前强制设置相同的种子
    # 这保证了 Epoch 50 的实验和 Epoch 100 的实验在初始化状态是完全一样的
...
EPOCH_SETTINGS = [50, 100, 200]  # 延展实验变量：不同的训练轮次
results = []

# === 循环不同的 Epoch 设置 ===
for epochs in EPOCH_SETTINGS:
    model_name = f"MyModel_Epochs_{epochs}"
